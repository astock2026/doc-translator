"""
Insert English translations into a .docx file as a new line within the same paragraph
as the Chinese text. Translations use Times New Roman, 10.5pt, non-italic.
Handles merged table cells by deduplicating based on XML element XPath.
...
Usage: python insert_translations.py <input.docx> <translations.json> [--output output.docx]
"""
import json
import sys
...
def _safe_para_text(para):
    ...
    try:
        ...
        # Fallback: manually join run texts, skipping None
        parts = []
...
def cell_has_existing_english_above(cell, target_pi):
    ...
    for pi in range(target_pi):
        ...
        if text and not has_chinese(text) and has_english(text):
            # Found an English-only paragraph above the target
            return True
    return False
...
def make_english_run(eng_text, is_first=False):
    """
    Create XML elements for English translation inline within a paragraph.
    Returns a list of lxml elements: [line_break, english_run]
    If is_first (no preceding Chinese text), omit the line break.
    """
    elements = []
    ...
    # Size: 10.5pt = 21 half-points
    sz = etree.SubElement(rPr, qn("w:sz"))
    sz.set(qn("w:val"), "21")
    szCs = etree.SubElement(rPr, qn("w:szCs"))
    szCs.set(qn("w:val"), "21")

    # Non-italic (explicitly set to false)
...
def insert_translations(input_path, translations_path, output_path=None):
    ...
    # --- Body paragraphs: add English run + line break within the same paragraph ---
    ...
    for i, p_elem in enumerate(paragraph_elements):
        if i in para_translations:
            ...
            # Check if paragraph already has content (Chinese text)
            existing_text = "".join(
    ...
    # --- Table cells: add English run within the same paragraph ---
    # Use XPath-based deduplication to handle merged cells (vertically/horizontally
    # merged cells share the same underlying XML element across multiple row/cell indices)
    modified_paths = set()
    table_inserted = 0
    skipped = 0
    ...
    for ti, translations_list in table_translations.items():
        ...
        for ri, ci, pi, eng_text in translations_list:
            ...
            if pi < len(cell.paragraphs):
                ...
                # Get unique XPath for this paragraph element
                try:
                    elem_path = tree.getpath(target_elem)
                except Exception:
                    elem_path = None
                # Skip if this paragraph was already modified (merged cell)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python insert_translations.py <input.docx> <translations.json> [--output output.docx]")
        sys.exit(1)
    inp = sys.argv[1]
    trans = sys.argv[2]
    out = None
    ...
    if "--output" in args:
        ...
        if idx + 1 < len(args):
            out = args[idx + 1]
    insert_translations(inp, trans, out)
